...
struct BoardHash
{
    std::size_t operator()(const std::vector<int> &board) const
    {
        std::size_t h = 0;
        std::hash<int> hi;
        for (int row : board)
...
{
    std::vector<double> V;
    std::vector<Action> player_policy;
    std::vector<PieceType> adv_policy;
    int iterations;
};
...
public:
    ...
    using PieceSelector = std::function<PieceType(const std::vector<int> &board, int step, std::mt19937 &rng)>;
    ...
    EvalStats evaluate_policy(const std::vector<Action> &policy, int episodes, std::mt19937 &rng, int max_steps = 1000) const;
    std::vector<Action> random_policy(std::mt19937 &rng) const;
    MinimaxResult minimax_value_iteration(double gamma = 0.99, int max_iter = 500, double tol = 1e-6) const;
    double initial_state_value_minimax(const std::vector<double> &V) const;
    EvalStats evaluate_minimax_strategies(const std::vector<Action> &player_policy, const std::vector<PieceType> &adv_policy, int episodes, int max_steps = 1000) const;
    std::vector<int> rollout_adversarial_states(const std::vector<Action> &player_policy, const std::vector<PieceType> &adv_policy, int max_steps = 1000) const;
    std::pair<int, int> successors_line_angle(int s_id, const Action &a, double &reward) const;
    ValueIterationResult value_iteration_with_piece_model(const std::function<double(const std::vector<int> &board)> &p_line, double gamma = 0.99, int max_iter = 500, double tol = 1e-6) const;
    EpisodeResult simulate_episode_custom(const std::vector<Action> &policy, const PieceSelector &selector, std::mt19937 &rng, int max_steps = 1000) const;
    EvalStats evaluate_policy_custom(const std::vector<Action> &policy, const PieceSelector &selector, int episodes, std::mt19937 &rng, int max_steps = 1000) const;
    PieceType adv_piece_for_board(const std::vector<int> &board, const std::vector<PieceType> &adv_policy) const;
...
private:
    ...
    int next_state_adversarial(int s_id, const Action &a, const std::vector<PieceType> &adv_policy, double &reward) const;
    int next_state_by_piece_choice(int s_id, const Action &a, PieceType chosen_piece, double &reward) const;
};
